...
let liveHistory = []; // Data သိမ်းမည့်နေရာ

// 1. Chart ကို ချက်ချင်းဆွဲမည်
setTimeout(() => {
    if (typeof TradingView !== 'undefined' && document.getElementById('tv_chart_container')) {
        new TradingView.widget({
          "width": "100%", "height": "100%", "symbol": "BINANCE:BTCUSDT", "interval": "1",
          "theme": "dark", "style": "1", "locale": "en", "toolbar_bg": "#f1f3f6",
          "enable_publishing": false, "hide_top_toolbar": true, "hide_side_toolbar": true,
          "container_id": "tv_chart_container", "backgroundColor": "#12121a"
        });
    }
}, 500);

// 2. Order တင်ရင် In Transaction ထဲပြ၊ ပြီးရင် Position Closed ထဲပြ
submitDeliveryOrder = function(type) {
  const amt = document.getElementById('deliveryAmount').value;
  if(!amt) { alert('Enter amount'); return; }
  ...
  const transDiv = document.getElementById('hist-content-transaction');
  transDiv.innerHTML = `
      <div style="background:#1e1e2d; padding:10px; border-radius:4px; margin-bottom:5px; border-left:3px solid #f39c12;">
          <div style="display:flex; justify-content:space-between; color:white;">
              <span>${type.toUpperCase()} (Counting down...)</span>
              <span>${currentDuration}s</span>
          </div>
          <div style="font-size:12px; color:#b2bec3;">Amount: ${amt}</div>
      </div>
  `;

  // Countdown ပြီးရင် Result ထုတ်မည်
  setTimeout(() => {
      // Fake Win/Loss Logic
      const isWin = Math.random() > 0.5;
      const pnl = isWin ? (amt * 0.5) : -amt;
      ...
      liveHistory.unshift({
          ...
          time: currentDuration,
          qty: amt,
          price: document.getElementById('mainPrice').innerText,
          pnl: pnl.toFixed(2),
          win: isWin,
      ...
      // Update UI
      renderClosedPositions(); // List ကို Refresh လုပ်မည်
      switchHistoryTab('closed'); // Closed Tab ကို ပြောင်းမည်
      transDiv.innerHTML = '<div style="text-align:center; color:#636e72; margin-top:20px;">No record yet</div>';
      
      alert(isWin ? "You Won!" : "You Lost!");

  }, currentDuration * 1000); // စက္ကန့်အတိုင်း စောင့်မည်
};

// 3. Render List (Live Data)
function renderClosedPositions() {
    const container = document.getElementById('hist-content-closed');
    if(!container) return;
    
    if(liveHistory.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#636e72; margin-top:20px;">No positions yet</div>';
        return;
    }

    container.innerHTML = liveHistory.map(item => `
        <div style="background:#1e1e2d; padding:10px; margin-bottom:5px; border-radius:4px; border-left:3px solid ${item.win ? '#00b894' : '#ff6b6b'};">
            <div style="display:flex; justify-content:space-between; font-size:12px; color:white;">
                <b>${item.symbol}</b>
                <span>${item.time}s</span>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:11px; margin-top:5px; color:#b2bec3;">
                <span>Qty: ${item.qty}</span>
                <span>Price: ${item.price}</span>
            </div>
            <div style="display:flex; justify-content:space-between; font-size:11px; margin-top:5px;">
                <span>${item.closeTime}</span>
                <span style="color:${item.win ? '#00b894' : '#ff6b6b'}; font-weight:bold;">
                    ${item.win ? '+' : ''}${item.pnl}
                </span>
            </div>
        </div>
...
switchHistoryTab = function(tabName) {
    const transDiv = document.getElementById('hist-content-transaction');
    const closedDiv = document.getElementById('hist-content-closed');
    const btns = document.querySelectorAll('.h-tab-btn');
    
    // Style update
    btns[0].style.borderBottom = tabName === 'transaction' ? '2px solid #00b894' : 'none';
    btns[0].style.color = tabName === 'transaction' ? 'white' : '#636e72';
    
    btns[1].style.borderBottom = tabName === 'closed' ? '2px solid #00b894' : 'none';
    btns[1].style.color = tabName === 'closed' ? 'white' : '#636e72';
